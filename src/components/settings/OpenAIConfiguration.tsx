...
import type { OpenAIProviderSetting, UserSettings } from "@/lib/schemas";
import { IpcClient } from "@/ipc/ipc_client";
import type { CodexCliInfo } from "@/ipc/utils/codex_cli_detector";

const DEFAULT_CLI_MODELS = ["gpt-5-codex", "gpt-5", "o4"];
...
export function OpenAIConfiguration({
  ...
  envVars,
  updateSettings,
}: OpenAIConfigurationProps) {
  const existing =
    (settings?.providerSettings?.openai as OpenAIProviderSetting) ?? undefined;

  const [apiKey, setApiKey] = useState(existing?.apiKey?.value ?? "");
  const [connectionMode, setConnectionMode] = useState<
    OpenAIProviderSetting["connectionMode"]
  ...
  const [cliFallbackToApi, setCliFallbackToApi] = useState(
    existing?.cliFallbackToApi ??
      ((existing?.connectionMode ?? "api") === "cli" ? false : true),
  );
  ...
  const [detectionResult, setDetectionResult] = useState<CodexCliInfo | null>(
    null,
  );
  ...
  const envCliPath = envVars["CODEX_CLI_PATH"];

  const hasSavedApiKey = Boolean(existing?.apiKey?.value);
  const apiStatus = useMemo(() => {
    if (connectionMode === "cli" && !cliFallbackToApi && !hasSavedApiKey) {
      return {
        variant: "default" as const,
        title: "Codex CLI mode",
        description:
          "CLI-only mode selected. An OpenAI API key is optional unless you enable API fallback.",
        icon: Info,
      };
    }
    ...
    if (hasSavedApiKey) {
      return {
        ...
        description:
          "Dyad will use the API key saved in settings for OpenAI models.",
        icon: CheckCircle2,
      };
    }
    if (envApiKey) {
      return {
        ...
        description:
          "OPENAI_API_KEY is set. Values saved below will override the environment variable.",
        icon: Info,
      };
    }
    return {
      ...
      description:
        "Provide an API key to enable API mode or fallback from Codex CLI when necessary.",
      icon: Info,
    };
  }, [hasSavedApiKey, envApiKey, connectionMode, cliFallbackToApi]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const openAiSettings: OpenAIProviderSetting = {
        ...(existing ?? {}),
        connectionMode,
      ...
      if (trimmedApiKey) {
        openAiSettings.apiKey = { value: trimmedApiKey };
      } else if (openAiSettings.apiKey) {
        delete openAiSettings.apiKey;
      }
  ...
  const handleDetect = async () => {
    ...
    setError(null);
    try {
      const result = await IpcClient.getInstance().detectCodexCli({
      ...
      setDetectionResult({
        path: null,
        isAvailable: false,
        errorMessage: e?.message ?? "Failed to check Codex CLI",
      });
    } finally {
      setDetecting(false);
    }
  ...
  const detectionStatus = useMemo(() => {
    ...
    if (detectionResult.isAvailable && detectionResult.path) {
      return (
        <Alert variant="default">
          ...
          <AlertTitle>Codex CLI detected</AlertTitle>
    ...
    return (
      <Alert variant="destructive">
        <AlertTitle>Codex CLI not available</AlertTitle>
        <AlertDescription>
          {detectionResult.errorMessage ||
            "Dyad could not find the Codex CLI. Check your installation or specify the executable path."}
        </AlertDescription>
      </Alert>
    );
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setConnectionMode(value as OpenAIProviderSetting["connectionMode"])
            }
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="cli">Codex CLI only</SelectItem>
              <SelectItem value="auto">
                Auto (CLI with API fallback)
              </SelectItem>
            </SelectContent>
          </Select>
          <p className="text-xs text-muted-foreground">
            Choose how Dyad connects to OpenAI. Auto mode prefers Codex CLI but
            falls back to the API when needed.
          </p>
        </div>
      ...
      {connectionMode !== "api" && (
        <div className="space-y-4 border border-border rounded-lg p-4">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              <h3 className="font-medium">Codex CLI Settings</h3>
              <p className="text-xs text-muted-foreground">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Optional: /usr/local/bin/codex"
                autoComplete="off"
              />
              <p className="text-xs text-muted-foreground">
                Environment variable: <code>CODEX_CLI_PATH</code>{" "}
            ...
            <div className="flex items-center justify-between border border-dashed rounded-md px-3 py-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  When CLI fails, automatically retry with the OpenAI API.
                </p>
              </div>
              <Switch
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="o4-mini, gpt-4.1-mini"
                className="min-h-20"
              />
            </div>
